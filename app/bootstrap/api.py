...
这里集中选择 AI gateway、数据库 session 和 outline facade 的具体实现，
避免接口层或业务层分散读取环境变量。
"""
...
def create_api_app():
    ...
    if database_configured:
        ...
        outline_facade = create_outline_service(ai_gateway, session_factory)
        logger.info(
            "api_outline_facade_configured",
            extra={"database_configured": database_configured, "outline_enabled": True},
        )
    else:
        # 数据库未配置时仍注册 HTTP 路由，由占位 facade 稳定返回 503。
        logger.warning(
            "api_outline_facade_unavailable",
            extra={
    ...
    app = build_http_app(
        ...
        outline_facade=outline_facade,
    )
    logger.info(
        "api_app_composition_completed",
        extra={
            "database_configured": database_configured,
            "outline_enabled": outline_facade is not None,
        },
    )
    return app
