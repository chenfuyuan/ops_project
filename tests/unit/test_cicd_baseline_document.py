...
class CicdBaselineDocumentTest(unittest.TestCase):
    ...
    def test_cicd_baseline_document_marks_all_default_governance_gates_as_blocking(
        self,
    ) -> None:
        root = Path(__file__).resolve().parents[2]
        contents = (root / "docs" / "cicd_baseline.md").read_text()
    ...
    def test_cicd_baseline_document_keeps_releasable_artifact_platform_neutral(
        self,
    ) -> None:
        root = Path(__file__).resolve().parents[2]
        contents = (root / "docs" / "cicd_baseline.md").read_text()
        ...
        self.assertIn(
            "不预设 GitHub Actions、GitLab CI、Jenkins 或其他平台实现已经存在", contents
        )

    def test_cicd_baseline_document_limits_cd_scope_to_controlled_deployment(
        self,
    ) -> None:
        root = Path(__file__).resolve().parents[2]
        contents = (root / "docs" / "cicd_baseline.md").read_text()
    ...
    def test_release_document_keeps_release_validation_and_rollback_scope(self) -> None:
        ...
        self.assertIn(
            "CI/CD 负责提供平台中立的阶段边界、默认门禁与受控部署入口", contents
        )
    ...
    def test_deployment_document_links_environment_admission_to_controlled_deployment(
        self,
    ) -> None:
        root = Path(__file__).resolve().parents[2]
