...
1. **检查交接摘要** — 检查当前对话上下文中是否存在来自 `/opsx:explore-req` 的交接摘要（见下方"消费交接摘要"）
2. **探索项目上下文** — 检查相关文件、文档、已有 OpenSpec change、最近的 commit（如果交接摘要已覆盖，可跳过已确认的部分）
3. **澄清需求与边界** — 每次一个问题，理解目标、约束、成功标准、非目标（优先从交接摘要的"未决问题"开始）
4. **判断范围是否适合单个 change** — 如果范围过大，先帮助用户拆分（如果交接摘要已给出拆分建议，直接确认）
5. **提出 2–3 种规划/架构方案** — 每个方案含一句话定位、具体做法、优劣势、适用场景；做显式对比（关键维度表格）；先亮推荐并说明在当前约束下为何最优（如果交接摘要已有推荐路径，可作为首选方案的起点）
6. **分段展示前置设计** — 按主题分段展示需求理解、架构方向、关键决策、OpenSpec 映射，并获得用户确认
7. **创建或复用目标 OpenSpec change** — 默认创建新 change；若已有明确匹配的活跃 change，则复用
8. **写入 pre_design 文档** — 默认写入 `pre_design.md`，必要时拆分同级 `pre_design.*.md`
9. **告知结果并结束** — 不自动进入后续 OpenSpec 流程
...
如果当前对话上下文中存在来自 `/opsx:explore-req` 的交接摘要（以 `## 交接摘要：` 开头的结构化文本），则按以下规则消费：

### 可直接继承的内容（不需要重新探索）

| 摘要字段 | 继承方式 |
|---------|---------|
| 需求定位 | 直接使用，不需要重新定位需求卡片 |
| 实现现状 | 直接使用，不需要重新扫描 app/ 和 archive/ |
| 依赖就绪度 | 直接使用，不需要重新检查 |
| 当前 change 范围 + 是否需要拆分 | 直接使用，跳过"判断范围是否适合单个 change"步骤 |

### 可作为起点但需深入的内容

| 摘要字段 | 使用方式 |
|---------|---------|
| 已确认的边界（目标、成功标准、范围内/外） | 作为已知基线，澄清时只聚焦未覆盖的维度 |
| 推荐实现路径 | 作为首选方案的起点，在规划/架构方案中优先展开 |

### 必须优先处理的内容

| 摘要字段 | 使用方式 |
|---------|---------|
| 未决问题 | 作为澄清阶段的优先问题列表，在其他澄清之前先处理 |
| 标注"待 pre-design 澄清"的字段 | 同上，纳入澄清优先队列 |

### 无交接摘要时

正常执行完整流程，从探索项目上下文开始。不要因为缺少摘要而跳过任何步骤。

## 流程详述

### 1. 探索项目上下文

**如果有交接摘要：** 跳过已确认的部分（需求定位、实现现状），只补充摘要未覆盖的上下文（如近期 commit、相关活跃 change）。

**如果没有交接摘要：** 做完整的轻量探索，目标是获得足够上下文来判断：
...
- **一句话定位** — 用一句话概括这个方案的核心思路，让用户无需读完细节就能区分方案
- **具体做法** — 关键模块怎么划分、数据怎么流转、接口怎么定义（不是抽象描述，是具体到可以判断的程度）
- **优势** — 这个方案在哪些维度上比其他方案更好
- **劣势 / 风险** — 这个方案的主要代价、技术风险、维护负担
- **适用场景** — 在什么条件下这个方案是最优选择

#### 方案之间必须做显式对比

不要让用户自己去逐个阅读后脑内比较。必须提供一个**关键差异对比**，至少覆盖以下维度（按需增减）：
...
对比时要具体。不要写"复杂度中等"这种模糊判断——写"需要新增 2 个 adapter + 1 个 port 接口，改动约 3 个现有文件"。

#### 推荐必须明确且有理由

- **先亮推荐** — 不要让用户读完所有方案才知道你推荐哪个
- **推荐理由不能只是优势罗列** — 必须说明"在当前项目的具体约束下，为什么这个方案比其他方案更合适"
- **说明什么条件下推荐会改变** — 例如"如果未来需要支持 X，方案 B 会成为更好的选择"
- **如果没有明显最优方案** — 诚实说明，列出决策关键点，让用户做最终判断

#### 反模式

- 不要三个方案只是复杂度递增的梯度（简单版 / 中等版 / 完整版）——方案之间应该是**思路不同**，不只是程度不同
- 不要为了凑数硬编第三个方案——如果只有两个有意义的方向，就只提两个
- 不要只列优缺点表格而不给结论——用户需要的是你的判断，不只是信息
- 不要在推荐中回避取舍——推荐方案一定有劣势，必须坦诚说出来
