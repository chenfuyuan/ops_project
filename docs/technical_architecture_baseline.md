...
| 本地运行 | Docker Compose | 作为推荐的本地编排基线，用于拉起依赖与多进程环境；当前仓库未必已提交对应文件 |
| 生产交付 | Containerized Deployment | 作为正式环境默认交付方向；当前仓库未必已包含镜像定义或部署清单 |
...
本地运行推荐采用 `Docker Compose` 作为编排基线，用于拉起：
...
当前仓库的正式文档将上述形态作为目标本地运行模型，但这并不等同于仓库已经提交了可直接运行的 `Dockerfile`、`docker-compose.yml` 或其他容器编排文件。若相关工程产物尚未落地，本节应理解为本地运行的推荐拓扑，而不是现成可执行资产。

如果项目初期尚未使用 Redis 或对象存储，也建议在本地基线中为其保留配置位和容器位，避免后续引入时破坏整体运行结构。

### 7.4 生产部署基线

生产环境默认以容器化部署作为正式交付方向，最小形态通常包括：
...
这里的“容器化部署”表达的是正式环境应遵循的交付基线，而不表示当前仓库已经包含镜像定义、部署清单或平台特定运行配置。
