...
| 组件 | 放置位置 | 理由 |
|------|---------|------|
| AI 网关 | `modules/ai_gateway/` | 有自己的领域（profiles、路由规则），通过 facade 对外服务 |
...
### 1.1 AI 网关 (AI Gateway)

**来源：** Phase 1 `02-ai-gateway.md`、Phase 5 `02-circuit-breaker.md`
...
AIGatewayAdapter 协议提供两个核心方法：`invoke`（同步调用，返回 AIGatewayResponse）和 `stream`（流式调用，返回异步字符串迭代器）。

AIGatewayRouter 在此基础上增加任务路由能力：根据 task_type 获取对应的 adapter，并提供带路由的 invoke/stream 方法。

此外提供 `extract_json` 辅助函数：接收 AIGatewayRouter、消息列表和 Pydantic schema，自动调用 AI 网关并解析为结构化对象，支持配置最大重试次数（默认 2 次）。
...
CircuitBreakerAIGatewayAdapter 包装 AIGatewayAdapter，自动集成熔断逻辑。
